    ...
    <div class="box-body">
        ...
        <table class='bon table-bordered tab_' >
            ...
            foreach ($transportBills as $transportBill){
                $totalTtc = $totalTtc +$transportBill['TransportBill']['total_ttc'] ;
                $totalTva = $totalTva +$transportBill['TransportBill']['total_tva'] ;
                $totalHt = $totalHt +$transportBill['TransportBill']['total_ht'] ;

                $totalAmountRemaining = $totalAmountRemaining + $transportBill['TransportBill']['amount_remaining'];
                ?>
                <?php if ($transportBillId == $transportBill['TransportBill']['id'] && $i == 1) {
                    $i ++ ;
                    ?>
                    <tr>
                        <td></td>
                        ...
                        <td></td>
                        <td><?php
                            $reglement = $transportBill['TransportBill']['total_ttc'] -$transportBill['TransportBill']['amount_remaining'];
                        ...
                        <td><?php echo number_format($transportBill['TransportBill']['amount_remaining'], 2, ",", $separatorAmount) ;
                            $reglement = 0;
                            ?>&nbsp;</td>
                        <td><?php echo number_format($transportBill['DetailPayment']['payroll_amount'], 2, ",", $separatorAmount) ?>&nbsp;</td>
                        <td><?php echo h($this->Time->format($transportBill['Payment']['receipt_date'], '%d-%m-%Y')); ?>&nbsp;</td>
                        <td><?php   switch($transportBill['Payment']['payment_type']){
                                case 1:
                                    echo __('Species');
                                    break;
                                case 2:
               ...
               <?php  } else {
                    ...
                    <tr>
                        ...
                        <td><?php echo h($this->Time->format($transportBill['TransportBill']['date'], '%d-%m-%Y')); ?>&nbsp;</td>
                        <td><?php echo h($transportBill['Supplier']['code']); ?>&nbsp;</td>
                        <td><?php echo h($transportBill['Supplier']['name']); ?>&nbsp;</td>
                        <td><?php echo number_format($transportBill['TransportBill']['total_ht'], 2, ",", $separatorAmount) ?>&nbsp;</td>
                        <td><?php echo number_format($transportBill['TransportBill']['total_tva'], 2, ",", $separatorAmount) ?>&nbsp;</td>
                        <td><?php echo number_format($transportBill['TransportBill']['total_ttc'], 2, ",", $separatorAmount) ?>&nbsp;</td>
                        <td><?php
                            $reglement = $transportBill['TransportBill']['total_ttc'] -$transportBill['TransportBill']['amount_remaining'];
                            echo number_format($reglement, 2, ",", $separatorAmount) ?>&nbsp;</td>
                        <td><?php echo number_format($transportBill['TransportBill']['amount_remaining'], 2, ",", $separatorAmount) ;
