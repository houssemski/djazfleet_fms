...
echo $this->Form->create('ProductFamily');
...
echo $this->Js->submit(__('Save'), array(  //create 'ajax' save button
    'update' => '#contentWrapFamily',  //id of DOM element to update with selector
    'class' => 'btn btn-primary',

    
    ));
if (false != $saved){ //will only be true if saved OK in controller from ajax save above
    $url = '/products/getFamilies/'.$family_id;
    
    echo "<script>
        jQuery('#dialogModalFamily').dialog('close');  //close containing dialog
        //location.reload();  //if you want to reload parent page to show updated user
       jQuery('#families').load('".$this->Html->url($url)."', function() {
