...
?>
    ...
    <div id="header">
        <div class="title"><?php echo __('Etat chauffeur') ?></div>
        <div class="title"><?php echo $sheetRideDetailRides[0]['Customer']['first_name'].' '.$sheetRideDetailRides[0]['Customer']['last_name']; ?></div>
    </div>
    ...
    <div class="box-body">
        <table class='bon table-bordered tab_' >
            ...
            foreach ($sheetRideDetailRides as $sheetRideDetailRide){?>
                <tr>
                    <td><?php echo h($this->Time->format($sheetRideDetailRide['SheetRideDetailRides']['real_start_date'], '%d-%m-%Y')); ?>&nbsp;</td>
                    <td><?php echo h($sheetRideDetailRide['Supplier']['code'].' '.$sheetRideDetailRide['Supplier']['name']); ?>&nbsp;</td>
                    <td><?php echo h($sheetRideDetailRide['DepartureDestination']['name'].' '.$sheetRideDetailRide['ArrivalDestination']['name']); ?>&nbsp;</td>
                    <td><?php echo h($sheetRideDetailRide['CarType']['name']); ?>&nbsp;</td>
                    <td><?php
                        $totalAmount = $totalAmount + $sheetRideDetailRide['Reservation']['cost'];
                        echo h(number_format($sheetRideDetailRide['Reservation']['cost'], 2, ",", $separatorAmount)); ?>&nbsp;</td>
                    <td><?php
                        $advance =   $sheetRideDetailRide['Reservation']['cost'] - $sheetRideDetailRide['TransportBillDetailRides']['unit_price'];
                        $totalAdvance = $totalAdvance + $advance ;
                        echo h(number_format($advance, 2, ",", $separatorAmount)); ?>&nbsp;</td>
                </tr>
            <?php }
