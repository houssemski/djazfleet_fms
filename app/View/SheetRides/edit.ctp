...
$this->end();
$this->request->data['SheetRide']['start_date'] = $this->Time->format($this->request->data['SheetRide']['start_date'], '%d-%m-%Y %H:%M');
$this->request->data['SheetRide']['real_start_date'] = $this->Time->format($this->request->data['SheetRide']['real_start_date'], '%d-%m-%Y %H:%M');
$this->request->data['SheetRide']['end_date'] = $this->Time->format($this->request->data['SheetRide']['end_date'], '%d-%m-%Y %H:%M');
$this->request->data['SheetRide']['real_end_date'] = $this->Time->format($this->request->data['SheetRide']['real_end_date'], '%d-%m-%Y %H:%M');
...
<h4 class="page-title"> <?= __('Edit sheet ride (travel)'); ?></h4>
<style>
    .filters {
        clear: both;
        width: 1050px;
...
<div class="box">
    <div class="edit form card-box p-b-0">
        ...
        <?php echo $this->Form->create('SheetRide', array('enctype' => 'multipart/form-data', 'onsubmit' => 'javascript:disable();'));
        echo $this->Form->input('id');
        if (!empty($transportBillDetailRide)) {
            $nb_rides++;
            $rowspan = ($nb_rides * 2) + 2;
        }
        ...
        <div class="box-body" style="max-width: 100%;">
            <table class="table table-bordered " id='table_rides'>
                <tr ><td colspan=2><?php echo __('At the establishment'); ?></td>
                    <td style="width: 95%;">
                        <div class="nav-tabs-custom pdg_btm">
                            ...
                            <div class="tab-content">
                                <div class="tab-pane active" id="tab_1">
                                    ...
                                    echo  $this->element('edit_departure_personal_transport');
                                    } else {
                                    echo  $this->element('edit_departure_utranx',array(
                                            ...
                                            if($permissionEditCarState  || $isAgent){
                                                ...
                                                if(!empty($sheetRideDepartureCarStates)){
                                                    ...
                                                    foreach ($sheetRideDepartureCarStates as $sheetRideDepartureCarState){
                                                        ...
                                                        echo '<div class="lbl4">' . $sheetRideDepartureCarState['CarState']['name'];
                                                        echo "</div>";
                                                        $options = array('1' => __('Yes'), '2' => __('No'));
                                                        $attributes = array('legend' => false, 'value'=>$sheetRideDepartureCarState['SheetRideCarState']['car_state_value']);
                                                        ...
                                                        echo "<div class=' col-sm-4'>" . $this->Form->input('SheetRideDepartureCarState.'.$i.'.note', array(
                                                                'label' => __('Note'),
                                                                'class' => 'form-control',
                                                                'value'=>$sheetRideDepartureCarState['SheetRideCarState']['note'],
                                                                'id' => 'note'.$i,
                                                                'empty' => '',
                                                            )) . "</div>";
                                                        echo "<div class=' input-file col-sm-4'>" . $this->Form->input('SheetRideDepartureCarState.' . $i . '.attachment', array(
                                                                ...
                                                                'value'=>$sheetRideDepartureCarState['SheetRideCarState']['attachment'],
                                                                'type' => 'file',
                                                                'empty' => ''
                                                            )) . "</div>";
                                                        echo $this->Form->input('SheetRideDepartureCarState.'.$i.'.car_state_id', array(
                                                            'id' => 'car_state_id'.$i,
                                                            'value'=>$sheetRideDepartureCarState['SheetRideCarState']['car_state_id'],
                                                            'type' => 'hidden',
                                                            'class' => 'form-filter '
                                                        )) ;
                                                        echo $this->Form->input('SheetRideDepartureCarState.'.$i.'.id', array(
                                                            'id' => 'car_state_id'.$i,
                                                            'value'=>$sheetRideDepartureCarState['SheetRideCarState']['id'],
                                                            'type' => 'hidden',
                                                            'class' => 'form-filter '
                                                        )) ;
                                                ...
                                                if(!empty($departureCarStates)){

                                                    foreach ($departureCarStates as $carState){
                                                        echo "<div style='min-height: 65px;'>";
                                                        echo "<div class=' col-sm-4' style='margin-top: 30px;'>";
                ...
                if($sheetRideWithMission ==1 || $sheetRideWithMission ==3) {
                    if (Configure::read("transport_personnel") == '1') {
                        echo $this->element('edit_sheet_ride_personal_transport');
                    } else {
                        if (Configure::read("gestion_commercial") == '1') {
                            if (($isDestinationRequired == 1)) {
                                echo $this->element('edit_sheet_ride_utranx');
                            } else {
                                echo $this->element('edit_sheet_ride_fms');
                            }
        ...
        <div class="nav-tabs-custom pdg_btm">
            ...
            <div class="tab-content">
                <div class="tab-pane active" id="tab_3">
                    ...
                    <?php
                    if (Configure::read("transport_personnel") == '1') {
                        echo  $this->element('edit_arrival_personal_transport');
                    } else {
                        echo  $this->element('edit_arrival_utranx');
                    } ?>
                    ...
                    if($permissionEditCarState || $isAgent){
                        ...
                        if(!empty($sheetRideArrivalCarStates)){
                            ...
                            foreach ($sheetRideArrivalCarStates as $sheetRideArrivalCarState){
                                ...
                                echo '<div class="lbl4">' . $sheetRideArrivalCarState['CarState']['name'];
                                echo "</div>";
                                $options = array('1' => __('Yes'), '2' => __('No'));
                                $attributes = array('legend' => false , 'value' => $sheetRideArrivalCarState['SheetRideCarState']['car_state_value']);
                                ...
                                echo "<div class=' col-sm-4'>" . $this->Form->input('SheetRideArrivalCarState.'.$i.'.note', array(
                                        'label' => __('Note'),
                                        'class' => 'form-control',
                                        'value' =>  $sheetRideArrivalCarState['SheetRideCarState']['note'],
                                        'id' => 'note'.$i,
                                        'empty' => '',
                                    )) . "</div>";
                                echo "<div class=' input-file col-sm-4'>" . $this->Form->input('SheetRideArrivalCarState.' . $i . '.attachment', array(
                                        ...
                                        'value' =>  $sheetRideArrivalCarState['SheetRideCarState']['attachment'],
                                        'class' => 'form-control filestyle',
                                        'type' => 'file',
                                        'empty' => ''
                                    )) . "</div>";
                                echo $this->Form->input('SheetRideArrivalCarState.'.$i.'.car_state_id', array(
                                    'id' => 'car_state_id'.$i,
                                    'value' =>  $sheetRideArrivalCarState['SheetRideCarState']['car_state_id'],
                                    'type' => 'hidden',
                                    'class' => 'form-filter '
                                ));
                                echo $this->Form->input('SheetRideArrivalCarState.'.$i.'.id', array(
                                    'id' => 'car_state_id'.$i,
                                    'value' =>  $sheetRideArrivalCarState['SheetRideCarState']['id'],
                                    'type' => 'hidden',
                                    'class' => 'form-filter '
                                ));
                            ...
                            if(!empty($arrivalCarStates)){

                                foreach ($arrivalCarStates as $carState){
                                    echo "<div style='min-height: 65px;'>";
                                    echo "<div class=' col-sm-4' style='margin-top: 30px;'>";
...
<script type="text/javascript">
    ...
    $(document).ready(function () {
        ...
        for (var i = 1; i <= nb_ride; i++) {
            ...
            if (jQuery('#car_subcontracting1').prop('checked')){
                var sheetRideDetailRideId = jQuery('#sheet_ride_detail_ride' + '' + i + '').val();
                jQuery('#cost_contractor_div' + '' + i + '').load('<?php echo $this->Html->url('/sheetRides/getSubcontractorCostBySheetRideDetailRide/')?>' + sheetRideDetailRideId+'/'+i, function () {
            ...
            // if (jQuery('#status' + '' + i + '').val() < 4) {

            if (jQuery('#source1' + '' + i + '').is(':checked')) {

                jQuery('#ride-div' + '' + i + '').addClass('select-inline2');
                var detail_ride_id = jQuery('#detail_ride' + '' + i + '').val();

                var type_id = jQuery("#car_type").val();

                if (type_id == '') type_id = 0;
                var transport_bill_detail_ride_id = jQuery('#transport_bill_detail_ride' + '' + i + '').val();
                var status = jQuery('#status' + '' + i + '').val();
                jQuery('#ride-div' + '' + i + '').load('<?php echo $this->Html->url('/sheetRides/getRidesFromCustomerOrder/')?>' + i + '/' + type_id + '/' + transport_bill_detail_ride_id + '/' + detail_ride_id+ '/' +status, function () {

                    jQuery('.select3').select2();
                    var id = 'km_arrival' + nb_ride;
                    ...
                    if (parseInt(i) - 1 == parseInt(nb_ride)) {
            ...
            if (jQuery('#source2' + '' + i + '').is(':checked')) {
                ...
                jQuery('#ride-div' + '' + i + '').load('<?php echo $this->Html->url('/sheetRides/getPersonalizedRidesFromCustomerOrder/')?>' + i + '/' + type_id + '/' + transport_bill_detail_ride_id +  '/' +status, function () {
            ...
            if(fieldMarchandiseRequired==1){
                var id = 'marchandiseClient' + i;

                if (jQuery('#sheet_ride_detail_ride' + i).length > 0) {
                    if (!$("#marchandise-div" + i).is(':empty')) {
                        getSavedMarchandises(id);
        ...
        if (jQuery('#cars').val() > 0) {
            /*  jQuery('#customers-div').load('
             echo $this->Html->url('/sheetRides/getCustomersByCar/')?>' + jQuery('#cars').val(), function () {
             jQuery('.select2').select2();

             }
             );
             jQuery('#remorques-div').load('
             echo $this->Html->url('/sheetRides/getRemorquesByCar/')?>' + jQuery('#cars').val(), function () {
             jQuery('.select2').select2();
             });*/
            jQuery('#consumption-bloc').load('<?php echo $this->Html->url('/sheetRides/getConsumptions/')?>' + jQuery('#cars').val(), function () {
                /* jQuery('#km-tank-departure-div').load('
                 echo $this->Html->url('/sheetRides/getKmAndTank/')?>' + jQuery('#cars').val(), function () {
                 /*calculKmDepartureEstimatedFirstRide();
                 calculKmArrivalEstimated('km_departure1');
                 estimateCost();
    ...
    i = jQuery('#nb_rides').val();
    if (jQuery('#source11').is(':checked')) {
    ...
    function removeMission(id) {
        ...
        if(jQuery('#sheet_ride_detail_ride' + '' + id + '').val()){
            var missionId = jQuery('#sheet_ride_detail_ride' + '' + id + '').val();
            var missionDeletedId = jQuery('#missions_deleted_id').val();

            if (missionDeletedId != '') {
                missionDeletedId = missionDeletedId + ',' + missionId;
            } else {
                missionDeletedId = missionId;
            }

            jQuery('#missions_deleted_id').val(missionDeletedId);
        }
        $('#depart' + id + '').remove();
        $('#arrive' + id + '').remove();
    ...
    function calculPlannedArrivalDate(id) {
        ...
        if (detail_ride >= 0) {
            ...
            if (averageSpeed > 0) {
                ...
                var nb_day = parseInt(jQuery("#duration_day" + '' + num + '').val());
                var nb_hour = parseInt(jQuery("#duration_hour" + '' + num + '').val());
                var nb_min = parseInt(jQuery("#duration_minute" + '' + num + '').val());
    ...
    function getInformationRide(id) {
        ...
        if (ride_id != '') {
            ...
            if (jQuery('#car_subcontracting1').prop('checked')){
                ...
                var carId = jQuery('#cars').val();
                jQuery('#cost_contractor_div' + '' + num + '').load('<?php echo $this->Html->url('/sheetRides/getContractCostByDetailRide/')?>' + ride_id + '/' + num + '/' + from_customer_order + '/' + carId);
            }
        }else {
            jQuery('#client-initial-div' + '' + num + '').load('<?php echo $this->Html->url('/sheetRides/getClientInitial/')?>' + num, function () {
                $('.select-search-client-initial').select2({
        ...
        $("#" + '' + id + '' + "-file").before(
            function () {
                if (!$(this).prev().hasClass('input-ghost')) {
                    var element = $("<input type='file' class='input-ghost' style='visibility:hidden; height:0 ;margin: 0;display: none;'>");
    ...
    function chooseFile(id) {
        ...
        $('#attachment' + '' + id + '').change(function () {

            $('#file' + '' + id + '').val($('#attachment' + '' + id + '').val());
    ...
    function calculateKmArrivalParc(id) {
        ...
        if (num == nb_ride) {
            ...
            jQuery("#distance-retour-parc").load('<?php echo $this->Html->url('/sheetRides/getDistanceReturnToParc/')?>' + detail_ride + '/' + from_customer_order+'/'+ arrivalDestinationId+'/'+carTypeId, function () {
                ...
                if ((jQuery('#origin_distance_retour').val() != "") && (jQuery('#destination_distance_retour').val() != "")) {
    ...
    function getSumDistance() {
        ...
        var distance = 0;
        if (jQuery('#distance_first').val()) {
            distance = parseFloat(jQuery('#distance_first').val());

        }

        for (j = 1; j <= nb_ride; j++) {
            if (jQuery('#distance' + '' + j + '').val()) {
            ...
            (jQuery('#departure_destination1').val() > 0 && jQuery('#arrival_destination1').val() > 0))
        {
            if (jQuery('#min_consumption').val() && jQuery('#max_consumption').val()) {
                ...
                if (jQuery('#departureTankStateMethod').val() == 1) {
                    liter_destination = liter_destination - parseFloat(jQuery('#reservoir').val());
                ...
                if (liter_destination < 0) {
                    liter_destination = 0;
                }

                if (jQuery('#km_liter').length) {
                    jQuery('#km_liter').val(liter_destination);
                }

            }
        }
        return liter_destination;
    }
    ...
    function verifyLiterTanks(id) {
        ...
        if (carId > 0) {
            ...
            jQuery('#consump_tank' + '' + i + '').load('<?php echo $this->Html->url('/sheetRides/verifyLiterTanks/')?>' + liter + '/' + i + '/' + carId, function () {
                ...
                if (liter > jQuery('#consumption_liter' + '' + i + '').val()) {
                }
    ...
    function removeConsumption(id) {
        ...
        var i = trId.substring(14, trId.length);
        // var i = id.substring(id.length - 1, id.length);
        if(jQuery('#consumptionId' + '' + i + '').val()){
            var consumptionId = jQuery('#consumptionId' + '' + i + '').val();
            var consumptionDeletedId = jQuery('#deleted_id').val();

            if (consumptionDeletedId != '') {
                consumptionDeletedId = consumptionDeletedId + ',' + consumptionId;
            } else {
                consumptionDeletedId = consumptionId;
            }

            jQuery('#deleted_id').val(consumptionDeletedId);
        }
    ...
    function getSavedMarchandises(id) {

        var trId = jQuery("#" + '' + id + '').parents('tr:first').attr('id');
        var num = trId.substring(6, trId.length);

        //var num = id.substring(17, id.length);
        var sheetRideDetailRideId = jQuery('#sheet_ride_detail_ride' + '' + num + '').val();
        var clientId = jQuery('#client' + '' + num + '').val();


        var fieldMarchandiseRequired = jQuery('#fieldMarchandiseRequired').val();
        jQuery('#marchandise-div' + '' + num + '').load('<?php echo $this->Html->url('/sheetRides/getSavedMarchandises/')?>' + clientId + '/' + num + '/' + sheetRideDetailRideId, function () {
            $(".select3").select2();
    ...
    function getAttachmentsByClient(id) {
        ...
        if (clientId > 0) {
            ...
            jQuery('#piece-div' + '' + num + '').load('<?php echo $this->Html->url('/sheetRides/getAttachmentsByClient/')?>' + clientId + '/' + num + '/' + sheetRideDetailRideId, function () {
                $(".select3").select2();
            });
        }
    ...
    function getLots(id) {
        ...
        var num = trId.substring(6, trId.length);
        var sheetRideDetailRideId = jQuery('#sheet_ride_detail_ride' + '' + num + '').val();
        jQuery('#lot-div' + '' + num + '').load('<?php echo $this->Html->url('/sheetRides/getLots/')?>' + num+ '/' + sheetRideDetailRideId, function () {
            $(".select3").select2();
        });
    }
    ...
    function getFinalSupplierByInitialSupplier(id) {
        ...
        if (typeof $('#'+id).val() != 'undefined') {
            ...
            var supplierFinalId = jQuery('#client_final' + '' + i + '').val();
            jQuery("#client-final-div" + '' + i + '').load('<?php echo $this->Html->url('/sheetRides/getFinalSupplierByInitialSupplier/')?>' + supplierId + '/' + i + '/' + supplierFinalId, function () {
        ...
        if(id == 'km_arr' && kmType=='arrival'){
            ...
            if(kmType=='arrival'){
                ...
                if(kmType=='departure' && id =='km_departure1'){
                    ...
                    var kmDeparture = jQuery('#km_arrival' + '' + i + '').val();
    ...
    function modifyInvoicedRide(i, value){
        jQuery('#invoiced_ride' + '' + i + '').val(value);
    }
</script>
