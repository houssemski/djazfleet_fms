...
?>
    ...
    <head>
        ...
        <style type="text/css">
            ...
            @page { margin: 95px 0; }
            #header { position: fixed; left: 0; top: -95px; right: 0; height: 150px; /*border-bottom: 1px solid #000;*/}
            ...
            .box-body{padding: 0; margin: 15px 0px 0px; width: 100%; position: relative !important;}
            .box-body2{
            ...
            #header table, #informations table,.total-dtab{
                padding:0px 50px;
            }
            #informations table{
    ...
    <div class="box-body">
        <div class="title"><?php echo __('Journal simplifié par detail effets') ?></div>
        <?php
        $finTable = false;
        $billId =0;
        $i =0;
        $nbProducts = count($billProducts);
        $totalHt =0;
        $totalTtc = 0;
        $totalTva = 0;
        $totalRistourne =0;
        ...
        if ($billId == $billProduct['Bill']['id'] ) {
            $totalHt = $totalHt + $billProduct['BillProduct']['price_ht'];
            $totalTtc = $totalTtc + $billProduct['BillProduct']['price_ttc'];
            $totalTva = $totalTtc- $totalHt;
            $totalRistourne = $totalRistourne + $billProduct['BillProduct']['ristourne_val'];
            ?>

        <tr>
            <td><?php echo h($billProduct['Product']['code'].'-'.$billProduct['Product']['name']); ?></td>
            <td><?php echo number_format($billProduct['BillProduct']['unit_price'], 2, ",", $separatorAmount) ?></td>
            <td><?php echo number_format($billProduct['BillProduct']['ristourne_val'], 2, ",", $separatorAmount) ?></td>
            <td><?php echo number_format($billProduct['BillProduct']['price_ht'], 2, ",", $separatorAmount) ?></td>
            <td><?php echo h($billProduct['Tva']['name']); ?></td>
            <td><?php echo number_format($billProduct['BillProduct']['price_ttc'], 2, ",", $separatorAmount) ?></td>
        </tr><?php
            if($i == $nbProducts ) { 
       ...
       <?php }
        ...
        <?php   $billId = $billProduct['Bill']['id'];
        ...
        <div class="box-body2">
            <div class="total-dtab">
                <div class="left-ct">
                    <span class="left"><strong><?php echo  __('Reference: '); ?></strong></span> <span class="right"> <?= $billProduct['Bill']['reference']?></span><br>
                    <span class="left"><strong><?php echo  __('Date: '); ?></strong></span> <span class="right"> <?= $this->Time->format($billProduct['Bill']['date'], '%d-%m-%Y') ?></span><br>
                </div>
                <div class="right-ct">
                    <span class="left"><strong><?php echo  __('Raison sociale: '); ?></strong></span> <span class="right"> <?= $billProduct['Supplier']['name'] ?></span><br>
                    <span class="left"><strong><?php echo  __('Code: '); ?></strong></span> <span class="right"> <?= $billProduct['Supplier']['name'] ?></span><br>
                </div>
        ...
        <table class='bon table-bordered tab_' >
            ...
            <thead >
            <tr>
                <th ><strong><?php echo __('Product'); ?></strong></th>
