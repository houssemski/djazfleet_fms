...
if($result) {
echo $this->Form->create('ProductUnit');
...
echo $this->Js->submit(__('Save'), array(  //create 'ajax' save button
    'update' => '#contentWrapUnit',  //id of DOM element to update with selector
    'class' => 'btn btn-primary',

    
    ));
if (false != $saved){ //will only be true if saved OK in controller from ajax save above
    $url = '/lots/getUnits/'.$unitId;
    
    echo "<script>
        jQuery('#dialogModalUnit').dialog('close');  //close containing dialog
        //location.reload();  //if you want to reload parent page to show updated user
       jQuery('#units').load('".$this->Html->url($url)."', function() {
