...
echo $this->Form->create('Carmodel' );
...
echo $this->Js->submit(__('Save'), array(  //create 'ajax' save button
    'update' => '#contentWrapModel',  //id of DOM element to update with selector
    'class' => 'btn btn-primary',

    
    ));
if (false != $saved){ //will only be true if saved OK in controller from ajax save above
    $url = '/cars/getModels/'.$mark_id.'/'.$model_id;
    echo "<script> 
        jQuery('#dialogModalModel').dialog('close');  //close containing dialog         
       jQuery('#model').load('".$this->Html->url($url)."', function(){
                jQuery('#model .select2').select2();
                });
    </script>";
    }
echo $this->Form->end();
