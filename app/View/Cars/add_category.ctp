...
if($result) {
    ...
    echo $this->Form->create('CarCategory');
    ...
    if (false != $saved) { //will only be true if saved OK in controller from ajax save above
        $url = '/cars/getCategories/' . $category_id;
        echo "<script>
        jQuery('#dialogModalCategory').dialog('close');  //close containing dialog         
       jQuery('#carcategories').load('" . $this->Html->url($url) . "', function(){
                jQuery('#carcategories .select2').select2();
                });
    </script>";
    }
    echo $this->Form->end();
