...
if($result){
   ...
   echo "<div class='form-group ' id='model'>" . $this->Form->input('carmodel_id', array(
    ...
    echo $this->Js->submit(__('Save'), array(  //create 'ajax' save button
        'update' => '#contentWrapRemorque',  //id of DOM element to update with selector
    ...
    if (false != $saved){ //will only be true if saved OK in controller from ajax save above
        $url = '/cars/getRemorques/'.$carId;

        echo "<script>
        jQuery('#dialogModalRemorque').dialog('close');  //close containing dialog         
       jQuery('#remorques-div').load('".$this->Html->url($url)."', function(){
                  $('.select-search-remorque').select2({
                        ajax: {
                            url: '". $this->Html->url('/cars/getRemorquesByKeyWord')."',
                            dataType: 'json',
                            delay: 250,
