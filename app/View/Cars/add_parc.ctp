...
echo $this->Js->submit(__('Save'), array(  //create 'ajax' save button
    'update' => '#contentWrapParc',  //id of DOM element to update with selector
...
if (false != $saved){ //will only be true if saved OK in controller from ajax save above
    $url = '/cars/getParcs/' . $parc_id;
    echo "<script>
        jQuery('#dialogModalParc').dialog('close');  //close containing dialog         
       jQuery('#parcs').load('".$this->Html->url($url)."', function(){
                jQuery('#parcs .select2').select2();
                });
    </script>";
    }
echo $this->Form->end();
