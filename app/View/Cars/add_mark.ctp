...
if (false != $saved){ //will only be true if saved OK in controller from ajax save above
    $url = '/cars/getMarks/'.$mark_id;
    $modelUrl = '/cars/getModels/'.$mark_id;
    $getModelUrl = '/cars/addModel/'.$mark_id;
    echo "<script>
       ...
       jQuery('#marks').load('".$this->Html->url($url)."', function(){
                jQuery('#marks .select2').select2();
                });
       jQuery('.overlayModel').attr('href', '".$this->Html->url($getModelUrl)."');
