...
echo $this->Js->submit(__('Save'), array(  //create 'ajax' save button
    'update' => '#contentWrapFuel',  //id of DOM element to update with selector
    'class' => 'btn btn-primary',

    
    ));
if (false != $saved){ //will only be true if saved OK in controller from ajax save above
    $url = '/cars/getFuels/'.$fuel_id;
    echo "<script>
        jQuery('#dialogModalFuel').dialog('close');  //close containing dialog         
       jQuery('#fuels').load('".$this->Html->url($url)."', function(){
                jQuery('#fuels .select2').select2();
                });
    </script>";
    }
echo $this->Form->end();
