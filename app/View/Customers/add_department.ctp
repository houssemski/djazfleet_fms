...
echo $this->Js->submit(__('Save'), array(  //create 'ajax' save button
    'update' => '#contentWrapDepartment',  //id of DOM element to update with selector
...
if (false != $saved){ //will only be true if saved OK in controller from ajax save above
    $url = '/customers/getDepartments/'.$department_id;
   
    echo "<script>
        jQuery('#dialogModalDepartment').dialog('close');  //close containing dialog        
        jQuery('#departments').load('".$this->Html->url($url)."', function() {
