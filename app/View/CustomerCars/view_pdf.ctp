<html>
    <head>
        ...
        <style type="text/css">
            ...
            #header { position: fixed; left: 0px; top: -95px; right: 0px; height: 90px; text-align: center; font-family: "Calibri",sans-serif;}
            #footer { position: fixed; left: 0px; bottom: -95px; right: 0px; height: 110px; font-family: "Calibri",sans-serif; line-height: 13px}
			.body-cond {font-size: 12px; color: #000080; margin-left: 10px; font-family: "Calibri",sans-serif;}
            .box-body{padding: 0px; margin: 10px; width: 100%; font-family: "Calibri",sans-serif; font-size: 13px;height:95%;line-height: 28px}
            ...
            .grand_titre{ font-size: 17px; font-weight : bold;}
			.petit_titre{ font-size: 13px; font-weight : bold;}
    ...
    <body>
		...
		<tr>
			...
			<td style='width:450px; text-align: right;'><img src="<?= WWW_ROOT ?>/img/reports/header_cvl.jpg"></td>
		</tr>
		</table>
        ...
        <div class="box-body">
           <table >
				<tr>
					...
					<br/><span style='margin-left:70px'>Lot Moutchachou locale n&deg;19 Hydra </span><br/>Repr&eacute;sent&eacute;e par son G&eacute;rant,Mr MECHEROUH Mohamed Tahar <br/>Et:</span></div>
					...
					<div><span class=' petit_titre'>Permis de Conduire N&deg;:</span> <span style='width:100px'><?=$customerCar['Customer']['driver_license_nu']?></span> <span class=' petit_titre'> Emis le :</span> <?=$this->Time->format($customerCar['Customer']['driver_license_date'], '%d/%m/%Y')?></div>
					<div><span class='petit_titre'>&agrave;:</span> <?= $customerCar['Customer']['driver_license_by'] ?></div>
					<div><span class='petit_titre'>C.N.I ou Passeport </span><span style='width=100px;'><?= $customerCar['Customer']['passport_nu']?></span><span class='petit_titre'> Emis le : </span><?=$this->Time->format($customerCar['Customer']['passport_date'], '%d/%m/%Y')?> </div>
					<div><span class='petit_titre'>&agrave;: </span><?= $customerCar['Customer']['passport_by']?></div>
					<div>Adresse permanente:  </div>
					<div>Autre Conducteur : </div>
					<div>Date  et Lieu de naissance:</div>
					<div>Permis de Conduire N&deg;:  Emis le : </div>
					<div>Observation: <?= $customerCar['CustomerCar']['obs']?></div>
					</td>
					<td> 
						<div>V&eacute;hicule:</div>
					<div>Marque :<?= $customerCar['Car']['Mark']['name'].' '.$customerCar['Car']['Carmodel']['name'] ?></div>
					<div>IMMATRICULE :<?= $customerCar['Car']['immatr_def']?> S�rie du Type N&deg;  : </div>
					<div>Energie : <?= $customerCar['Car']['Fuel']['name']  ?></div>
					<div>Kilom&eacute;trage</div>
					<div>Km d&eacute;part: <?=$customerCar['CustomerCar']['km']?>  Km arriv&eacute;e: <?=$customerCar['CustomerCar']['next_km'] ?></div>
					<?php 
					$km_parcouru=$customerCar['CustomerCar']['next_km']- $customerCar['CustomerCar']['km']
					
					?>
					<div>Km parcouru <?= $km_parcouru ?></div>
					<div>NB: La Location comprend un forfait de 300 Km par jour <br/>
					...
					<div>D&eacute;part/ Arriv&eacute;e</div>
					<div>Date et heure de d&eacute;part:  <?=$this->Time->format($customerCar['CustomerCar']['start'], '%d/%m/%Y %H:%M')?> </div>
					<div>Date et heure d'arriv&eacute;e: <?=$this->Time->format($customerCar['CustomerCar']['end'], '%d/%m/%Y %H:%M')?>  </div>
					<?php  
					$nb_jour = $customerCar['CustomerCar']['end']-$customerCar['CustomerCar']['start'];
					?>
					<div>Nbr Jours: <?=  $nb_jour ?></div>
					<div>Livraison &agrave; : </div>
					<div>Reprise &agrave; : </div>
					<div> Prolongation</div>
					<div>Du :        heure D&eacute;part : </div>
					<div>Du :        heure Arriv&eacute;e : </div>
					<div></div>
					</td>
				</tr>
		...
		<br/>
		<br/>		
        <div style='color: #000080; border: dotted; margin-left:15px;margin-right:15px; padding:5px'> Le  V&eacute;hicule mis � votre disposition  propre et v&eacute;rifi&eacute;, devra &ecirc;tre restitu&eacute; dans les m�me condition sous peine de facturation de 1000 DA ( pour son n&eacute;ttoyage )
		</div>
   ...
   <div id="footer">
                ...
                <?= $company['Company']['adress'] ?>
                    ...
                    <tr>
                        <td class="first">
                            AI : <?= $company['Company']['ai'] ?> - NIF : <?= $company['Company']['nif'] ?>
                        </td>
                        <td class="second">
			...
			<div style='font-size: 12px;'>	Annexe CVL Blida: Hotel Ville des Roses    Boulevard Mohamed Boudiaf    (bvd 20m�tres)   Blida        Mob:+213(0) 558 481 852      E-mail:cvl_villedesroses@yahoo.fr </div>
            </div>       
...
<div class='body-cond'>
<div class="conditions_title">Condition g�n�rale de location</div> 
<p>ARTICLE 1 : </p>
