...
<head>
    ...
    <style type="text/css">
        ...
        <?php }else{ ?>
        @page { margin: 0px 0; }
        #header {left: 0; top: -95px; right: 0; height: 140px; /*border-bottom: 1px solid #000;*/}
        <?php } ?>
...
<div class="box-body">
    ...
    <div class="title"><?php echo __('Reservation per supplier') ?></div>
    <table class='bon table-bordered tab_' >
        ...
        <?php
        $costSupplier = 0;
        $amountRemainingSupplier = 0;
        $supplierId = $reservations[0]['Supplier']['id'];
        ...
        foreach($reservations as $reservation){
            ...
            $costSupplier = $costSupplier + $reservation['Reservation']['cost'];
            $amountRemainingSupplier = $amountRemainingSupplier + $reservation['Reservation']['amount_remaining'];
            ...
            if($i<$nbReservation){
                if($supplierId != $reservations[$i]['Supplier']['id']) { ?>
                    <tr>
                        <td style ='padding: 10px 5px 10px 70%; text-align: left;'colspan="4" >
                            <span  style ='display: inline-block; width: 50%;'><strong><?php echo  __('Supplier cost: '); ?></strong></span>
                            <span class="right"> <?= number_format($costSupplier, 2, ",", $separatorAmount).' '. $this->Session->read("currency");?></span>
                            <br/>
                            <span  style ='display: inline-block; width: 50%;'> <strong><?php echo  __('Left to pay: '); ?></strong></span>
                            <span>  <?php echo number_format($amountRemainingSupplier, 2, ",", $separatorAmount) .' '. $this->Session->read("currency");?></span>
                        </td>
                    </tr>
                    <?php

                    $supplierId = $reservation['Supplier']['id'];
                    $costSupplier = 0;
                    $amountRemainingSupplier = 0;
                    ?>
