...
if($result){
    ...
    echo $this->Js->submit(__('Save'), array(  //create 'ajax' save button
        'update' => '#contentWrapWilaya',  //id of DOM element to update with selector
        'class' => 'btn btn-primary',
    ));

    if (false != $saved){ //will only be true if saved OK in controller from ajax save above
        $url = '/dairas/getWilayas/'.$wilaya_id;
        echo "<script>
        jQuery('#dialogModalWilaya').dialog('close');  //close containing dialog
       jQuery('#wilayas').load('".$this->Html->url($url)."', function() {
