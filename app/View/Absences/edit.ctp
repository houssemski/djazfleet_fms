...
<?php
$this->request->data['Absence']['start_date'] = $this->Time->format($this->request->data['Absence']['start_date'], '%d-%m-%Y');
$this->request->data['Absence']['end_date'] = $this->Time->format($this->request->data['Absence']['end_date'], '%d-%m-%Y');

?><h4 class="page-title"> <?= __('Edit') . ' ' . __('absence'); ?></h4>
    <div class="box">
        <div class="edit form card-box p-b-0">
            <?php echo $this->Form->create('Absence', array('onsubmit' => 'javascript:disable();')); ?>
            <div class="box-body">
                <?php
                echo $this->Form->input('id');
                echo $this->Tinymce->input('Absence.description', array(
                    'label' => 'Description',
                ...
                echo "<div class='form-group input-button' id='reason-div'>" . $this->Form->input('absence_reason_id', array(
                ...
                <div class="btn-group quick-actions">
                    <div id="dialogModalReason">
                        <!-- the external content is loaded inside this tag -->
                        <div id="contentWrapReason"></div>
                    </div>
                    ...
                    <ul class="dropdown-menu">
                        <li>
                            <?php echo $this->Html->link('<i class="fa fa-plus m-r-5"></i>' . __('Add', true),
                                array("controller" => "absenceReasons", "action" => "addReason"),
                                array(
                                    "class" => "btn overlayReason",
                                    'escape' => false,
                                    "title" => __("Add reason")
                                )); ?>
                        </li>
                        <li>
                            <a href="#" class="btn overlayEditReason" title=<?php echo __("Edit reason") ?>>
            ...
            <?php    echo "<div class='form-group'>" . $this->Form->input('start_date', array(
                ...
                <div id='attachment-file'>
                    ...
                    echo "<div class='form-group input-button4' >" . $this->Form->input('file', array(
                            ...
                            'readonly' => true,
                            'id' => 'file',
                            'value' => $this->request->data['Absence']['attachment'],
                            'class' => 'form-control',
                        )) . '</div>';
...
<?= $this->Html->script('bootstrap-filestyle'); ?>
    <script type="text/javascript">
        ...
        $(document).ready(function () {
            ...
            jQuery("#dialogModalReason").dialog({
                autoOpen: false,
                height: 300,
                width: 400,
                show: {
            ...
            jQuery(".overlayReason").click(function (event) {
                event.preventDefault();
                jQuery('#contentWrapReason').load(jQuery(this).attr("href"));
                var reasonDiv = jQuery('#dialogModalReason');
                reasonDiv.dialog('option', 'title', jQuery(this).attr("title"));
                reasonDiv.dialog('open');
            });

            jQuery(".overlayEditReason").click(function (event) {

                event.preventDefault();
                var reasonId = jQuery("#reason").val();

                if (reasonId > 0) {
                    var reasonDiv = jQuery('#dialogModalReason');
                    reasonDiv.dialog('option', 'title', jQuery(this).attr("title"));
                    reasonDiv.dialog('open');
                    jQuery('#contentWrapReason').load("<?php echo $this->Html->url('/absenceReasons/editReason/')?>" + reasonId  );
                }
            });
